...
abstract class CartViewModelBase {
  ...
  void updateCheckoutTotal(BuildContext context);
}
...
class CartViewModel extends CartViewModelBase {
  ...
  @override
  increment(int index) {
    cartModel.items[index].subTotal =
        cartModel.items[index].subTotal + cartModel.items[index].price;
  ...
  @override
  decrement(int index) {
    if (cartModel.items[index].qty != 0) {
      cartModel.items[index].subTotal =
          cartModel.items[index].subTotal - cartModel.items[index].price;
      cartModel.items[index].qty--;
      cartModel.total = cartModel.total - cartModel.items[index].price;
  ...
  @override
  void updateCheckoutTotal(BuildContext context) {
    CheckoutTotalState.of(context).updateTotalValue();
  }
}
